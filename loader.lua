...
local Linkvertise_Link =
    'https://ads.luarmor.net/get_key?for=Vail_Hub_Key-lgGHKWosXGmT'

local PlaceIDs = {
    [12355337193] = 'c19180f7c85232d08ac0ee9b3c606e2d',
    [13771457545] = 'c19180f7c85232d08ac0ee9b3c606e2d',
    [863266079] = 'bfb70d091deb1255fc2d91122876022a',
}
...
local function createLoaderUI()
    ...
    MainFrame.Size = UDim2.fromOffset(350, 240)
    ...
    Instance.new('UIStroke', KeyInput).Color = Color3.fromRGB(80, 80, 90)
    KeyInput.Parent = MainFrame
    local SubmitButton = Instance.new('TextButton')
    SubmitButton.Size = UDim2.new(1, -40, 0, 35)
    SubmitButton.Position = UDim2.new(0.5, 0, 0.6, 0)
    ...
    GetKeyButton.Size = UDim2.new(0.5, -25, 0, 25)
    GetKeyButton.Position = UDim2.new(0.25, 0, 0.8, 0)
    GetKeyButton.AnchorPoint = Vector2.new(0.5, 0.5)
    GetKeyButton.BackgroundTransparency = 1
    GetKeyButton.Text = 'Get Key'
    GetKeyButton.Font = Enum.Font.Gotham
    GetKeyButton.TextSize = 14
    GetKeyButton.TextColor3 = Color3.fromRGB(120, 255, 160)
    GetKeyButton.Parent = MainFrame
    local DiscordButton = Instance.new('TextButton')
    DiscordButton.Size = UDim2.new(0.5, -25, 0, 25)
    DiscordButton.Position = UDim2.new(0.75, 0, 0.8, 0)
    DiscordButton.AnchorPoint = Vector2.new(0.5, 0.5)
    DiscordButton.BackgroundTransparency = 1
    DiscordButton.Text = 'Join Discord'
    DiscordButton.Font = Enum.Font.Gotham
    DiscordButton.TextSize = 14
    DiscordButton.TextColor3 = Color3.fromRGB(120, 160, 255)
    ...
    SubmitButton.MouseButton1Click:Connect(function()
        ...
        if #user_key < 16 then
            ...
            StatusLabel.Text = 'Error: Invalid key format.'
            return
        end
        ...
        local loader_url = 'https://api.luarmor.net/files/v3/loaders/'
            .. current_script_id
            .. '.lua'
        ...
        if not success then
            warn(
                'Vail Hub Main Script failed to execute. Error:',
                tostring(err)
            )
        end
    ...
    task.wait(0.1)
    TweenService
        :Create(MainFrame, TweenInfo.new(0.5), { BackgroundTransparency = 0 })
        :Play()
    for _, child in ipairs(MainFrame:GetChildren()) do
        if child:IsA('GuiObject') then
            local bgTrans = 0.8
            if child == SubmitButton then
                bgTrans = 0
            end
            if child == DiscordButton or child == GetKeyButton then
                bgTrans = 1
            end
            TweenService
                :Create(
                    child,
                    TweenInfo.new(0.5),
                    { BackgroundTransparency = bgTrans, TextTransparency = 0 }
                )
                :Play()
        end
    end
